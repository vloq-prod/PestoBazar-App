...
interface PriceSliderProps {
  ...
  initialMin?: number; // ✅ for filter persist
  initialMax?: number; // ✅ for filter persist
}
...
const PriceSlider = ({
  ...
  const initMinPos = (initialMin / PRICE_MAX) * SLIDER_WIDTH;
  const initMaxPos = (initialMax / PRICE_MAX) * SLIDER_WIDTH;
  ...
  const pxPerStep = SLIDER_WIDTH / ((PRICE_MAX - PRICE_MIN) / STEP);

  // ── Worklet helpers ────────────────────────────────────────
  const getPrice = (pos: number) => {
    'worklet';
    return PRICE_MIN + Math.floor(pos / pxPerStep) * STEP;
  };
  ...
  const trackTap = Gesture.Tap().onEnd(e => {
    ...
    if (distMin <= distMax) {
      ...
      const clamped = Math.max(tapX, position.value + pxPerStep);
  ...
  const pan = Gesture.Pan()
    ...
    .onEnd(() => {
      opacity.value = 0;
      runOnJS(notifyChange)(getPrice(position.value), getPrice(position2.value));
    });
  ...
  const handleMinInput = useCallback(
    (text: string) => {
      ...
      setMinValue(clamped);
      position.value = withTiming((clamped / PRICE_MAX) * SLIDER_WIDTH, {duration: 200});
      onValueChange?.({min: clamped, max: maxValue});
    },
    [maxValue, onValueChange, position],
  );
  ...
  const handleMaxInput = useCallback(
    (text: string) => {
      ...
      setMaxValue(clamped);
      position2.value = withTiming((clamped / PRICE_MAX) * SLIDER_WIDTH, {duration: 200});
      onValueChange?.({min: minValue, max: clamped});
    },
    [minValue, onValueChange, position2],
  );
      ...
      <GestureDetector gesture={trackTap}>
        <View style={[styles.sliderContainer, {width: SLIDER_WIDTH}]}>
          ...
          <View
            style={[
              styles.trackBack,
              {backgroundColor: colors.primaryLight, width: SLIDER_WIDTH},
            ]}
          />
