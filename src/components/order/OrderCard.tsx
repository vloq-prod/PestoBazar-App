...
const STATUS_CONFIG: Record<string, { color: string; bg: string; icon: any; text: string }> =
  {
    "Order Placed": { color: "#6B7280", bg: "#F3F4F6", icon: ShoppingBag, text: "Order placed" },
    "Pickup Scheduled": { color: "#6B7280", bg: "#F3F4F6", icon: Clock, text: "Pickup scheduled" },
    "Product Dispatched": { color: "#3B82F6", bg: "#EFF6FF", icon: Package, text: "Product dispatched" },
    "On Delivery": { color: "#3B82F6", bg: "#EFF6FF", icon: Truck, text: "On delivery" },
    "Product Delivered": {
      color: "#10B981",
      bg: "#ECFDF5",
      icon: CheckCircle2,
    ...
    "Cancelled": { color: "#EF4444", bg: "#FEF2F2", icon: XCircle, text: "Order cancelled" },
  };

export const getStatusConfig = (status: string) =>
  STATUS_CONFIG[status] ?? { color: "#6B7280", bg: "#F3F4F6", icon: Package, text: status || "Order update" };
...
const OrderCard = ({
  ...
  const handleInvoicePress = () => {
    // invoice download functionality placeholder/trigger
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.headerSection}>
        ...
        >
          ₹{parseFloat(item.order_amount).toFixed(0)}
        </Text>
      </View>
